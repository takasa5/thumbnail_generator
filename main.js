...
$(document).ready(function () {
    (function (w, d, f) {
        var defaultCheckString = "!\"#$%&'()*+,-./0123456789:;<=>?@ABCDEFGHIJKLMNOPQRSTUVWXYZ[\\]^_`abcdefghijklmnopqrstuvwxyz{|}~";
        var regex = /"/g;
        f = f.replace(regex, '\"');
        ...
        /**
         * 指定したフォントがインストールされているかどうかを返す
         * @param {string} [font] - フォント名
         * @param {string} [checkString] - 確認する文字列
         * @returns {boolean}
         */
        function isInstalledFont(font, checkString) {
            element.innerHTML = toString(checkString) || defaultCheckString;
            element.style.fontFamily = '"' + toString(font).replace(regex, '\"') + '",' + f;
        ...
        w.isInstalledFont = isInstalledFont;
    })(window, document, 'Blank');
    ...
    ctx1.font = "normal 30px 'Sawarabi Mincho'";
    ...
    putCharVertically($("#textBox").val());

    $("#numBox").bind('keyup mouseup', function() {
        putCharVertically($("#textBox").val());
    });
    $("#startX").bind('keyup mouseup', function() {
        putCharVertically($("#textBox").val());
    });
    $("#startY").bind('keyup mouseup', function() {
        putCharVertically($("#textBox").val());
    });

    var image = new FileReader();
    image.addEventListener('load', function() {
        $(".inner").append('<img id="rimg" src="' + image.result + '">');
...
function trimming() {
    var rimg = $("#rimg").cropper("getCroppedCanvas");
    $(".modal").hide();
    var canvas = document.getElementById("cvs");
    var ctx = canvas.getContext('2d');
    ctx.drawImage(rimg, 640, 0, 640, 720);
    $("#rimg").cropper("destroy");
    $("#rimg").remove();
}

function putCharVertically(val) {
    var canvas = document.getElementById("cvs");
    var ctx = canvas.getContext('2d');
    var fontSize = Number($("#numBox").val());
    var hp = 0;
    var hps = 0;
    var vp = 0;
    var vps = Number($("#startY").val());
    var chr = '';
    ctx.fillStyle = "black";
    ctx.fillRect(0, 0, 640, 720);
    if (isInstalledFont("Yu Mincho", val))
        ctx.font = fontSize + "px Yu Mincho";
    else if (isInstalledFont("YuMincho", val))
        ctx.font = fontSize + "px YuMincho";
    else if (isInstalledFont("MS Mincho", val))
        ctx.font = fontSize + "px MS Mincho";
    else if (isInstalledFont('HiraMinProN-W6', val))
        ctx.font = fontSize + "px HiraMinProN-W6";
    else
        ctx.font = fontSize + "px Sawarabi Mincho";
    ctx.fillStyle = "white";
    for (var i = 0; i < val.length; i++) {
        ...
        if (chr == "\n") {
            hps += fontSize;
            vps = Number($("#startY").val());
        } else {
            if (chr.match(/[、。]/)) {
                hp = 0;
            } else {
                hp = ctx.measureText(chr).width;
            }
            vp = fontSize;
            if (!(Number($("#startX").val()) - hps + fontSize + hp / 2> 640))
                ctx.fillText(chr, Number($("#startX").val()) - hps + fontSize - hp / 2, fontSize + vps);
            vps = vps + vp;
            if (vps > canvas.height - fontSize / 2) {
                hps += fontSize;
                vps = Number($("#startY").val());
            }
        }
